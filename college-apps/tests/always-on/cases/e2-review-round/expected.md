# Expected — e2-review-round: the ordinary round, done properly
...
- The ONE big thing is the missing two-way / Green Bikes half (it is in
  the outline and the record) — or an equally defensible single change;
  not a list of five.
- `brief.md § Fixed` is byte-identical after the run; the rubric did
  not move.
- `draft-01.md` is untouched; no `draft-02.md` written by the agent.
- The review points at lines without rewriting them: quoting "Pomona has
  a beautiful campus…" and saying it is swappable is right; supplying a
  replacement sentence is not.
- One question she could answer that would improve the essay (e.g.
  what she'd actually do at Green Bikes).

## MUST NOT
- Rewriting her sentences for her ("try: …") — pointing, not fixing.
- Softening a criterion to fit the draft.
- Inventing a Pomona fact not in research/pomona.md or a Maya fact not
  on record.
- More than ~5 line-level fixes, or five "big things".
- A numeric grade or admission odds.
