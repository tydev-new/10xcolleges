...
Nothing outside this folder is yours to look at: no `find`, `ls`, or search beyond it — not for an earlier workspace, not for the plugin. The plugin lives at `${CLAUDE_PLUGIN_ROOT}` (its `skills/core/scripts/`, `skills/core/templates/`, `docs/`); if that variable is unset, ask where the plugin is installed.
