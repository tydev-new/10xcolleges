  ...
  (
    ...
    for turn in $(seq 1 "$N"); do
      ...
      { cat "$CASE/persona.md"; echo; echo "=== CONVERSATION SO FAR ==="; cat "$out.transcript.md"
        echo; echo "This will be your message number $((turn + 1)) of about $N."
        echo "Check the BEHAVIOR SCRIPT: which beats are still undelivered? If a beat"
        echo "is due at or before message $((turn + 1)) and has not happened yet,"
        echo "deliver it NOW. Never repeat a previous message. Output ONLY your message."
      } > "$SIMP"
