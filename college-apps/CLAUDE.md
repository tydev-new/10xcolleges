...
- `${CLAUDE_PLUGIN_ROOT}/skills/core/references/data-model.md` — the binding data contract, including what
  may change each file. Check a file's mutability class (when it may change, and how)
  before writing to it.

## Working style

Read `${CLAUDE_PLUGIN_ROOT}/skills/core/references/voice.md` before talking to a student. Plain-spoken, encouraging, specific,
...
skills/core/config/             calendar.json — dates and offsets, editable without touching code
skills/core/scripts/            deterministic generators (Scorecard, xlsx, docx, HTML package)
...
skills/core/templates/student/  scaffold for a new student
```

**JSON everywhere** — `${CLAUDE_PLUGIN_ROOT}/skills/core/config/calendar.json`, `meta.json`, `packet.json`. One format,
...
weeks) belong in `${CLAUDE_PLUGIN_ROOT}/skills/core/config/calendar.json`. Rules for *computing* dates — which aid year a
...
- **Run scripts with `python3 "${CLAUDE_PLUGIN_ROOT}/skills/core/scripts/<name>.py"`.** They find
...
- **Cite every college fact** with its source and vintage (the year, or for a deadline the date it was checked). `${CLAUDE_PLUGIN_ROOT}/skills/core/references/citations.md` is binding.
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/core/scripts/scorecard.py" search "Case Western"            # find UNITID
python3 "${CLAUDE_PLUGIN_ROOT}/skills/core/scripts/scorecard.py" get --unitid 201645,170976       # batch, 1 request
python3 "${CLAUDE_PLUGIN_ROOT}/skills/core/scripts/scorecard.py" quota
