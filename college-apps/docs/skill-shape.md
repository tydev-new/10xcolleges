...
skill fires (`templates/workspace-CLAUDE.md`).
...
skill whose mechanical checks live in a script it does not host needs no
`scripts/` of its own, and its `eval.md § Who checks what` names the
script it uses. Here the scripts are shared at the plugin root
(essay-coach runs `scripts/check_draft.py`, student-intake runs
...
states its scope**: `eval.md` evaluates the whole skill; content scoped
narrower gets its scope in a SECTION name (`eval.md § The rubric`),
never a stealth-scoped file wearing the generic name.
...
3. **The loops and sequences**, with a short intro that says how you
   ...
   **A `###` section is a loop or sequence the skill runs; a procedure
   ...
   - Any read the loop's RULES depend on is a **step that says "read
     X"** — a parenthetical citation is how a source-repo loop ran
...
3. **Rewire every inbound pointer** — grep the old filenames across
   ...
   `docs/` — except dated records (`docs/evals/*.md`, everything under
   `tests/always-on/results/`, superseded design docs, banner-marked),
   which describe what WAS and are never rewired. Then sweep **section names too**: `§` pointers and "below"
   references break without any filename changing, and
   `docs/data-model.md` links to schema sections by exact name.
...
6. Run `python3 -m unittest discover tests`, the kit invariants
   ...
   count against what you expect** (§ schema.md above). Growth into the loaded tier
   is the shape's known cost — reported, never discovered.
...
- **The judge reads the tool log.** Every call the agent made is in
  ...
  against calls, and a claim with no call is a claim.
...
- **A skill's loops are specified in its own `SKILL.md` — never in a
  ...
  **verbatim at each loop's exit, unmarked** — short, identical copies
  guarded by the invariants test, no echo bookkeeping. The one-copy
...
- **When a loop detaches from a shared reference**, two things travel
  with it or the detach is incomplete: the shared LAW ("a draft never
  justifies changing the rubric; a criterion relaxed to fit what was
  written has stopped being a standard") lands **unmarked at the loop's
  exit** — the source repo measured that a pointer alone does not carry
  it — and the residual text is **trimmed in the same commit** (and
  removed when its last consumer leaves): a tier row, a budget, or a
  known-gap line left behind becomes a second authoritative copy the
...
- **The loop shape is stated per skill** (essay-coach's essay loop,
  student-intake's interview) — the sanctioned duplication, guarded by
